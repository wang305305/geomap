...
const map = new mapboxgl.Map({
  container: 'map',
  style: 'mapbox://styles/oliviawu/ckfq4rdl9129819l2hscm7xrn',
  pitch: 50,
  center: [-70,70],
  zoom: 1.55,
});
...
scene.on('loaded', () => {
  ...
  Promise.all([
    ...
    //fetch('dot.json').then(d => console.log(d.text())),
...
*/
  ...
  const dotData = eval(dotjson);
  const flydata = eval(linejson).map(item => {
    const latlng1 = item.from.split(',').map(e => { return e * 1; });
    const latlng2 = item.to.split(',').map(e => { return e * 1; });
    return { coord: [ latlng1, latlng2 ] };
  });
